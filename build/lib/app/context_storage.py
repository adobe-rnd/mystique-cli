...
class ContextStorage:
    def __init__(self, llm_client: LlmClient, persist_directory=".vector_db"):
        self.llm_client = llm_client
        self.client = chromadb.PersistentClient(settings=Settings(persist_directory=persist_directory, allow_reset=True))
        self.files_collection = self.client.get_or_create_collection(name="files_collection")
    ...
    def get_similar_files(self, prompt: str, n: int = 5) -> List[str]:
        ...
        results = self.files_collection.query(query_embeddings=[prompt_vector], n_results=n)
...
if __name__ == "__main__":
    ...
    context_db.store_file(file_path="example.txt", content="This is an example content.")

    similar_files = context_db.get_similar_files(prompt="example prompt")
    print(similar_files)

    print("Is the database empty?", context_db.is_empty())
